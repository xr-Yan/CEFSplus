...
def tt_svm(X,y):
    ...
    for k in range(10):
        ...
        for train_index, test_index in cv.split(X):
            ...
            hist = clf.fit(X_train, y_train, eval_set=[(X_test, y_test)])
            # print(len(y[train]))
...
fs_df = pd.read_excel('Lasso.xlsx')
...
def ssssvm(k,f):
    ...
    if m > 0:
        ...
        plt.legend()
        s = "my_plot_lgb_" + str(k) + '_' + str(m) + '.png'
...
n_jobs = 12
u = Parallel(n_jobs=n_jobs)(delayed(ssssvm)(k,f=10) for k in range(1))
print(u)
uuu = pd.DataFrame(u)
uuu.to_csv('cz2_11_lgb_df.csv')
