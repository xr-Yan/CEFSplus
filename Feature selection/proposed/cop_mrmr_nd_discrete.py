...
data = pd.read_csv('all_t_pure.csv',low_memory=False)
...
def comput_part_2(x,s_s,i): #x,ls,ln = x，已选特征的名字列表，单个未选特征的名字，返回l是未选特征的第二部分列表

    huhuhu_c = c20(i,s_s,x)
    #print(huhuhu_c)
    return huhuhu_c
...
#l = Parallel(n_jobs=12)(delayed(comput_part_2)(x,ls,i) for i in ln)
#huhuhu--每个特征与标签
#l--每个特征与已选特征
"""l = comput_part_2(x,ls,ln)
...
def compu_3(xy1,ls,i):
    l = ls
    l.append(i)
    #print(l)
    c1 = copent.copent(xy1[:, 1])
    l.append(-1)
    #print(l)
    c2 = copent.copent(xy1[:,l])
...
def mrmi(xy,k,n_jobs = 12): #xy--x&y的阵，最后一列是y（标签），k--要选的特征个数

    x = xy[:, :-1]  # "###前20个先尝试"
    y = xy[:, -1]

    length = x.shape[1]
    huhuhu = Parallel(n_jobs=n_jobs)(delayed(cc)(i, xy) for i in range(length))
    #print(huhuhu)
    part1 = json_dump(huhuhu)
    part1.to_csv('new_R_part1.csv')

    nphu = np.append(huhuhu)
    s_s = nphu.argmax()
    ls = [s_s]
    ...
    ls_select.to_csv('ls_select_ge.csv')
...
"""
20, 795, 735, 603, 57, 0, 1890, 2277, 149, 1911"""